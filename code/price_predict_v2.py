...
check_point = ModelCheckpoint('price_model_v2.hdf5', monitor = "val_loss", mode = "min", save_best_only = True, verbose = 1)

history = model.fit(X_train, y, batch_size = 10240, epochs = 30,verbose = 1, validation_split=0.1,callbacks=[early_stop,check_point])

model.load_weights('price_model_v2.hdf5')
preds = model.predict(X_nan,verbose=1)

test_id["price_pred_onlydescription"]=preds.reshape(-1)
test_id.to_csv('../input/label_price_features_onlydescription.csv',index=None)

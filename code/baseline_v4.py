...
item_features=pd.read_csv("../input/image_meta.csv")
...
for cols in ["title","description"]:
    data_small[cols + '_num_words'] = data_small[cols].apply(lambda comment: len(str(comment).split()))
    ui_features.append(cols + '_num_words')
...
valid=data_small[(data_small.activation_date>="2017-03-15")&(data_small.activation_date<="2017-03-21")].copy().reset_index(drop=True)
train=data_small[(data_small.activation_date>="2017-03-22")&(data_small.activation_date<="2017-03-28")].copy().reset_index(drop=True)
